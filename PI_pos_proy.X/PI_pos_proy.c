 ...
 * File: PI_pos_proy.c
 *
 * Code generated for Simulink model 'PI_pos_proy'.
 *
 * Model version                  : 1.40
 * Simulink Coder version         : 8.4 (R2013a) 13-Feb-2013
 * C/C++ source code generated on : Sat Nov 12 23:46:50 2016
 */

#include "PI_pos_proy.h"
#include "PI_pos_proy_private.h"

/* Block signals (auto storage) */
B_PI_pos_proy_T PI_pos_proy_B;

/* Block states (auto storage) */
DW_PI_pos_proy_T PI_pos_proy_DW;

/* Real-time model */
RT_MODEL_PI_pos_proy_T PI_pos_proy_M_;
RT_MODEL_PI_pos_proy_T *const PI_pos_proy_M = &PI_pos_proy_M_;

/* Model step function for TID0 */
void PI_pos_proy_step0(void)           /* Sample time: [0.001s, 0.0s] */
{
  ...
  PI_pos_proy_B.QEI1 = POSCNT;         /* Read QEI  position */

  /* Step: '<Root>/Step' */
  if (((PI_pos_proy_M->Timing.clockTick0) * 0.001) < 10.0) {
    rtb_Abs = 0.0;
  } else {
    rtb_Abs = 0.2618;
  }
   ...
   *  Gain: '<Root>/Gain2'
   */
  rtb_Gain2 = 212.630236019562 * rtb_Abs - (real_T)PI_pos_proy_B.QEI1;

  /* Gain: '<S1>/Proportional Gain' incorporates:
   *  DiscreteIntegrator: '<S1>/Integrator'
   *  Sum: '<S1>/Sum'
   */
  rtb_Abs = (rtb_Gain2 + PI_pos_proy_DW.Integrator_DSTATE) * 0.19;
  ...
  if (u >= 1740U) {
    PI_pos_proy_B.U2CH1_m = 1740U;
  } else if (u <= 10U) {
    PI_pos_proy_B.U2CH1_m = 10U;
  } else {
    PI_pos_proy_B.U2CH1_m = u;
  }
  ...
  PI_pos_proy_DW.Integrator_DSTATE += 0.001 * rtb_Gain2;

  /* Update for S-Function (MCHP_PWM): '<Root>/PWM1' */
  PDC1 = PI_pos_proy_B.U2CH1_m;        /* Period for Channel 1 */
   ...
   */
  PI_pos_proy_M->Timing.clockTick0++;
}

/* Model step function for TID1 */
void PI_pos_proy_step1(void)           /* Sample time: [0.01s, 0.0s] */
{
  /* S-Function (MCHP_QEI): '<Root>/QEI' */
  PI_pos_proy_B.U2CH1_g = POSCNT;      /* Read QEI  position */

  /* DataTypeConversion: '<Root>/Data Type Conversion1' incorporates:
   *  DataTypeConversion: '<Root>/Data Type Conversion'
   *  Gain: '<Root>/Gain3'
   */
  PI_pos_proy_B.U2CH1 = (int16_T)fmod((int16_T)floor(0.004703 * (real_T)
    PI_pos_proy_B.U2CH1_g), 65536.0);
    ...
    do {
      switch (index) {
       case 0:                         /* CH 1 */
        ...
        MCHP_UART2_Tx.buffer[MCHP_UART2_Tx.tail] = PI_pos_proy_B.U2CH1 ;/* LSB First*/
        MCHP_UART2_Tx.tail = (MCHP_UART2_Tx.tail + 1) & (Tx_BUFF_SIZE_Uart2 - 1);
        MCHP_UART2_Tx.buffer[MCHP_UART2_Tx.tail] = (PI_pos_proy_B.U2CH1 >> 8);/* MSB Last*/
        MCHP_UART2_Tx.tail = (MCHP_UART2_Tx.tail + 1) & (Tx_BUFF_SIZE_Uart2 - 1);
        if (++index >= 1)
          index = 0;
      }
...
void PI_pos_proy_initialize(void)
{
  /* Registration code */
  ...
  (void) memset((void *)PI_pos_proy_M, 0,
                sizeof(RT_MODEL_PI_pos_proy_T));
  (PI_pos_proy_M)->Timing.TaskCounters.cLimit[0] = 1;
  (PI_pos_proy_M)->Timing.TaskCounters.cLimit[1] = 10;

  /* block I/O */
  (void) memset(((void *) &PI_pos_proy_B), 0,
                sizeof(B_PI_pos_proy_T));

  /* states (dwork) */
  (void) memset((void *)&PI_pos_proy_DW, 0,
                sizeof(DW_PI_pos_proy_T));
